...
@section('content')
    ...
    {{-- Contenu principale de ma page create.blade.php --}}
    <div class="bg-gray-100 shadow-lg rounded-lg border p-6">
        ...
        {{-- Permet d'avoir un message de succés --}}
        @if (session('success'))
            <div class="alert alert-success text-green-600">{{ session('success') }}</div>
        @endif
        
        {{-- Formulaire permettant la création d'un compte --}}
        <form action="{{ route('accounts.store') }}" method="POST">
            ...
            <ul class="space-y-3">
                ...
                <li>
                    ...
                    <input value="{{ old('Password_Account') }}" type="password" name="Password_Account"
                        id="Password_Account" class="border-gray-700 w-full px-4 py-2 border rounded-lg hover:bg-yellow-50"
                        required>
                </li>
                <li>
                    <label for="First_name_Account" class="block text-gray-700"><strong>Prénom :</strong></label>
                    <input value="{{ old('First_name_Account') }}" type="text" name="First_name_Account"
                ...
                <li>
                    ...
                    <input value="{{ old('Last_name_Account') }}" type="text" name="Last_name_Account"
                        id="Last_name_Account" class="border-gray-700 w-full px-4 py-2 border rounded-lg hover:bg-yellow-50"
                        required>
                </li>

                <li>
                    <label for="Birth_date_Account" class="block text-gray-700"><strong>Date de naissance :</strong></label>
                    <input value="{{ old('Birth_date_Account') }}" type="date" name="Birth_date_Account"
                        id="Birth_date_Account"
                        class="border-gray-700 w-full px-4 py-2 border rounded-lg hover:bg-yellow-50" required>
                </li>

                <li>
                    <label for="Id_Role" class="block text-gray-700"><strong>Rôle :</strong></label>
                    <select value="{{ old('Id_Role') }}" name="Id_Role" id="Id_Role"
                        class="border-gray-700 w-full px-4 py-2 border rounded-lg hover:bg-yellow-50" required>
                ...
                <li class="mx-auto w-fit">
                    <button type="submit"
                        class="text-yellow-500 px-4 py-2 rounded h-min border-yellow-500 border-2 hover:border-green-500 hover:bg-green-300 hover:text-black">
